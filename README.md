...
- Cached player summaries, selected-scope loading, and explicit private/unavailable states. A lobby uses paced per-player stats and profile requests, plus shared season and leaderboard requests; switching scope loads the other scope. The public build makes no batch request during lobby loading.
